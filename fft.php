...
function chance($cookie, $ua, $target, $jumlah= 3, $sleep){
    ...
    $listids = array();
		...
		for($i=0;$i<count($listids);$i++):
			if($private[$i] == "1"){
			    echo "@".$namanya[$i]." [>] Akunnya di Private BGSD [SKIP] <br>";
			}else{
			    $show = request(1, $ua, 'friendships/show/'.$listids[$i].'/', $cookies);
                    ...
                    echo " @".$namanya[$i]." [>] ".$follow.",".$hasilkomen.",".$hasillike." [".$terpilih."] <br>";
                    }else{
			        echo "@".$namanya[$i]." [>] Timeline Kosong BGSD [SKIP] <br>";
                    }
                    }else{
                    echo "@".$namanya[$i]." [>] Sudah Di Follow <br>";
                    }
...
for ($x = 0; $x <= $jumlah; $x++){
	$ib = chance($go->cookie, $go->ua, $target, $jumlah, $sleep);
    echo ' '. date("H:i:s").  ' '" ".$ib. "\n";
}
